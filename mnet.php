...
/**
 * Strings for component 'mnet', language 'ko', branch 'MOODLE_39_STABLE'
 *
 * @package   mnet
...
$string['name'] = '이름';
$string['net'] = '네트워크';
$string['networksettings'] = '네트워크 설정';
$string['never'] = '접속 없음';
...
$string['requiresopenssl'] = '네트워크는 OpenSSL 확장이 필요';
$string['restore'] = '복구';
...
$string['ssoacldescr'] = '원격 MNet 호스트로부터 특정 사용자에게 접속을 승인/거부 하려면 이 페이지를 이용하십시요.  원격 사용자에게 SSO 서비스를 제공하면 가능하다. 여러분의 로컬 사용자가 다른 무들네트워크 호스트를 탐방할 수 있게 제어하려면, 역할 시스템에서 그들에게 <em>mnetcanroam</em> 능력을 부여하면 된다.';
...
$string['trustedhostsexplain'] = '<p>인증된 호스트 매카니즘은 무들 API의 일부분에 XML-RPC를 통하여 호출을 실행 특정 서버에서 실행시킬 수 있도록 합니다. 이는 스크립트가 무들의 동작을 제어하게 하는데, 활성화 시키기에는 매우 위험한 선택 사항이 될 것입니다. 만일 확신이 서지 않는다면, 이를 꺼 두도록 하십시요.</p>
 ...
 <p>이것을 활성화시키기 위해서는, 한 줄에 하나씩 IP 주소 혹은 네트워크 목록을 입력하십시요. 다음은 작성 예입니다.</p>로칼 호스트:<br />127.0.0.1<br />로칼 호스트(네트워크 블록):<br />127.0.0.1/32<br />IP 주소가 192.168.0.7인 호스트만:<br />192.168.0.7/32<br />IP 주소가 192.168.0.1 에서 192.168.0.255 사이의 모든 호스트:<br />192.168.0.0/24<br />모든 호스트:<br />192.168.0.0/0<br />물론 마지막 예는 추천할 수 <strong>없는</strong> 사례입니다.';
$string['turnitoff'] = '끔';
$string['turniton'] = '켬';
$string['type'] = '유형';
...
$string['validated_by'] = '<code>{$a}</code> 네트워크에 의해 승인되었습니다.';
